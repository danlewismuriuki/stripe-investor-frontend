...
//         // customerId // Now customerId is guaranteed to be a string
//       );
...
export function PaymentDashboard() {
  ...
  const connectedAccountId = user?.connectedAccountId; // Ensure this is available
  ...
  useEffect(() => {
    ...
    async function fetchPaymentMethods() {
      try {
        const res = await fetch(
          `/api/stripe/connected-payment-methods/${connectedAccountId}`
        );
        const data = await res.json();
        setPaymentMethods(data);
      } catch (error) {
